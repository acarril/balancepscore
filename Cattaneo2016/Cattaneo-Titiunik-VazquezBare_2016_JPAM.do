...
** RDROBUST: net install rdrobust, from(http://www-personal.umich.edu/~cattaneo/software/rdrobust/stata) replace
** RDDENSITY: net install rddensity, from(http://www-personal.umich.edu/~cattaneo/software/rddensity/stata) replace
** RDLOCRAND: net install rdlocrand, from(http://www-personal.umich.edu/~cattaneo/software/rdlocrand/stata) replace
********************************************************************************
********************************************************************************
clear all
set more off
...
capture log close
log using mainpaper_log, replace text
...
preserve
	drop _all
	svmat T
	outsheet using output/table_bintest.csv, comma replace
restore
...
mat T[3,3] = e(n_l)
mat T[3,4] = e(n_r)
mat T[3,5] = round(e(pval_q),.001)

preserve
	drop _all
	svmat T
	outsheet using output/table_densitytests.csv, comma replace
restore
...
preserve
	drop _all
	svmat T
	outsheet using output/table_flexpoly.csv, comma replace
restore
...
preserve
	drop _all
	svmat T
	outsheet using output/table_rdrobust.csv, comma replace
restore
...
preserve
	drop _all
	svmat T
	outsheet using output/table_rdrandinf.csv, comma replace
restore
...
rdsensitivity $Y R, wlist(0.3(0.2)10.1) tlist(-7(.25)2) saving(output/graphdata)
preserve
...
rdsensitivity $Y R, wlist(0.3(0.2)10.1) tlist(-7(.25)2) p(1) saving(output/graphdata_p1)
preserve
use output/graphdata_p1, clear
twoway contour pvalue t w, ccuts(0(0.05)1) ccolors(gray*0.01 gray*0.05 ///
	gray*0.1 gray*0.15 gray*0.2 gray*0.25 gray*0.3 gray*0.35 ///
	gray*0.4 gray*0.5 gray*0.6 gray*0.7 gray*0.8 gray*0.9 gray ///
	black*0.5  black*0.6 black*0.7 black*0.8 black*0.9 black) ///
...
preserve
	drop _all
	svmat T
	outsheet using output/table_rdrandinf-CI.csv, comma replace
restore
...
preserve
	drop _all
	svmat T
	outsheet using output/table_rosenbaum.csv, comma replace
restore
...
preserve
	drop _all
	svmat T
	outsheet using output--supplement/table_summaryresults.csv, comma replace
restore
...
mat rownames time = "Figure1" "Table1" "Table2" "Figure2" "Table3" "Figure3" "Table4" "Table5" "Figure5"
mat colnames time = "time"
