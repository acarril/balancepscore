program define balancepscore1, rclass byable(recall)
	version 11.1
...
qui test `int_test'
local l=`s'-1
local pvalue_`s'= 1-F(r(df),r(df_r),r(F))
if `pvalue_`s''>`pvalue_`l'' {
local numblock= `optblock'
}
else {
local pvalue_`s'=`pvalue_`l''
}
qui drop `block`s''  `blockid' 
foreach x in `cov' {
qui drop `block`s'_`x''
}
...
di in ye	     "**************************************************** "
	...
	local j=`j'+1  
		...
		matrix colnames `orbal' = "Mean `G0'" "Mean `G1'" "StMeanDif" p-value 
