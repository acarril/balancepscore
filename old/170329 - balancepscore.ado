...
version 11.1
syntax varlist [if] [in] , [pscore(string) weight(string) comsup logit latex dir(string) namgroup(string) bdec(int 3) addnamtex(string)]
...
**Observations:
preserve
qui keep if  `touse2' & `T'==0
local N0=_N
restore
preserve
qui keep if `touse2' & `T'==1
local N1=_N
restore
...
qui reg `var'  `T0' `T'  if `touse2', noconstant
mat m=r(table)
...
**Observations:
preserve
qui keep if  `touse' & `T'==0
local Nweight0=_N
restore
preserve
qui keep if  `touse' & `T'==1
local Nweight1=_N
restore

* compute weights

qui sum `T' if `touse' & `T'==1
local hd=r(N)
qui sum high_direct if `touse' & `T'==0
local ld=r(N)
qui gen `weight'=`hd'/(`hd'+`ld')/`pscore'*(`T'==1) + `ld'/(`hd'+`ld')/(1-`pscore')*(`T'==0) if `touse'
...
qui reg `var' `T0' `T' [iw=`weight'] if `touse' , noconstant
mat m=r(table)
...
di in ye             "**************************************************** "
di in ye	     "Propensity score-weighted "
di in ye	     "**************************************************** "
...
if `"`dir'"' != `""'  {
texdoc init `dir'\BalanceImprove_`T'`addnamtex'.tex, replace
}
else {
texdoc init BalanceImprove_`T'`addnamtex'.tex, replace
}
tex \\ \\ [-1.5ex]
