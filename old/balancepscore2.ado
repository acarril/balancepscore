program define balancepscore2, rclass byable(recall)
	version 11.1
syntax varlist [fweight iweight pweight] [if] [in] , [pscore(string) blockid(string) numblock(int 5) optblock comsup logit latex(string) dir(string) namgroup(string) bdec(int 3)]
...
*** Maximun number of block possible:
...
*** Observation by block
forval t=0/1 {
forval i=1/`aux1'  {
qui summarize `T' if `T'==`t' & `blockid'==`i' 
local m`i'`t'=r(N)
if `m`i'`t''<=1  {
local aux2=`aux1'
local maxblock=`aux1'-1
}
}
}
cap drop `blockid'
...
foreach i of numlist 1(1)`optblock' {
local mean`j'1: di `mean`j'1' + (m`i'[`j',1]*`Total`i'')/`TotalSample'
local mean`j'2: di `mean`j'2' + (m`i'[`j',2]*`Total`i'')/`TotalSample'
}

*Std mean difference
local stmeandiff`s'`j': di (`mean`j'1'-`mean`j'2')/overall[`j',1]
}
...
local k=0 
forvalue j=1/`numcov'{ 
foreach x  of numlist `stmeandiff`s'`j'' {
 local k=abs(`x')+`k' 
 } 
 }
 
 local l=`s'-1
local stmeandiff`s'=`k'/`numcov'

if `stmeandiff`s''<`stmeandiff`l'' {
local numblock= `optblock'
}
else {
local stmeandiff`s'=`stmeandiff`l''
}
qui drop  `blockid' 
...
di in ye	     "**************************************************** "
	...
	local j=`j'+1  
		...
		matrix colnames `balimp' = "Mean `G0'" "Mean `G1'" "StMeanDif" p-value
